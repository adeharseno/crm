...
function load_script($url, $arr_add = array()){
	if(file_exists($url)) $script=file_get_contents($url);
	else return "Script not found!";
	
	$script=str_replace("\r\n", "", $script);
	$script=str_replace("\t", "", $script);
	if(count($arr_add)>0) 
		$script = str_replace(array_keys($arr_add), array_values($arr_add), $script);
	return $script;
}

function set_option($arr, $opt_selected = "", $mode = "") {
    $s_option = "";
    foreach ($arr as $key => $value) {
        ...
        if (!is_array($value)) {
            ...
            if ($mode == "")
                $s_option.="<option value=\"" . $key . "\" " . $selected . ">" . /*ucwords(strtolower(*/$value/*))*/ . "</option>";
            else
                $s_option.="<option value=\"" . $value . "\"" . $selected . ">" . /*ucwords(strtolower(*/$value/*))*/ . "</option>";
        }else {
            foreach ($value as $key1 => $value1) {
                $selected = "";
                if ($mode == "") {
                    if ($opt_selected == $key1)
                        $selected = "selected";
                }
                else {
                    if ($opt_selected == $value1)
                        $selected = "selected";
                }

                if ($mode == "")
                    $s_option.="<option value=\"" . $key1 . "\" " . $selected . ">-. " . /*ucwords(strtolower(*/$value1/*))*/ . "</option>";
                else
                    $s_option.="<option value=\"" . $value1 . "\"" . $selected . ">-. " . /*ucwords(strtolower(*/$value1/*))*/ . "</option>";
            }
        }
    }
    return $s_option;
